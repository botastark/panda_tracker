...
The only supported task-pose packet (wire version 2) contains:
...
The receiver ignores duplicate and older sequence numbers. Ignored
...
Check that the sender and receiver both use port `6501` and that the
versioned 148-byte task-pose packet is being sent:
...
underlying estimate. Increment the sequence only for a genuinely new tracker
estimate.
