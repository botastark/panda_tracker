...
def remove_generated_pbvs_elements(root: ET.Element) -> None:
    """Remove stale PBVS elements if a generated XML is reused as input."""
    for parent in list(root.iter()):
        for child in list(parent):
            if child.get("name", "").startswith("pbvs_"):
                parent.remove(child)
...
def add_visual_box_leg(
    ...
    delta = end - start
    if abs(delta[0]) < 1e-6:
        raise ValueError(f"{name} must have non-zero length along H x.")
    if abs(delta[1]) > 1e-9 or abs(delta[2]) > 1e-9:
        raise ValueError(
            f"{name} must be straight along H x and perpendicular to the rod."
        )
    center = 0.5 * (start + end)
    half_size = np.array(
        [0.5 * abs(delta[0]), half_thickness, half_thickness],
        dtype=float,
    )
    ET.SubElement(parent, "geom", {
        "name": name,
        "type": "box",
        "pos": vector_string(center),
        "size": vector_string(half_size),
        "rgba": rgba,
        "contype": "0",
        "conaffinity": "0",
        "group": "2",
...
def create_scene_xml(
    ...
    """Add the measured holder, stick, camera, and triangle geometry."""
    tree = ET.parse(panda_xml)
    root = tree.getroot()
    remove_generated_pbvs_elements(root)
    worldbody = root.find("worldbody")
    if worldbody is None:
        raise ValueError(f"{panda_xml} has no <worldbody> element.")

    ee_body = find_body_by_name(root, ee_body_name)
    if ee_body is None and ee_body_name == "pbvs_ee":
        hand_body = find_body_by_name(root, "hand")
        if hand_body is None:
            raise ValueError(
                "Cannot create virtual E frame because body 'hand' "
                "was not found."
            )
        T_hand_E = config_transform(
            tool_visualization,
            "T_mujoco_hand_E",
        )
        ee_body = ET.SubElement(
            ...
            {
                "name": "pbvs_ee",
                "pos": vector_string(T_hand_E[:3, 3]),
                "quat": quaternion_string(T_hand_E[:3, :3]),
            },
        )
    if ee_body is None:
    ...
    holder_center_H = config_vector(
        tool_visualization, "holder_center_H", [0.0, 0.0, 0.0]
    )
    holder_color = str(tool_visualization.get("holder_rgba", ".55 .55 .60 1"))
    show_debug_frames = bool(
        tool_visualization.get("show_debug_frames", False)
    )
    ...
    stick_mount_H = config_vector(
        tool_visualization, "stick_mount_H", [0.0, 0.0, 0.045]
    )
    rod_end_H = config_vector(
        tool_visualization,
        "rod_end_H",
        [0.0, 0.0, 0.179],
    )
    sphere_center_H = config_vector(
        tool_visualization,
        "sphere_center_H",
        [0.0, 0.0, 0.186],
    )
    rod_half_size = config_vector(
        tool_visualization,
        "rod_half_size",
        [0.005, 0.005, 0.067],
    )
    support_1_holder_H = config_vector(
        tool_visualization,
        "camera_support_1_holder_H",
        [0.010, -0.012, -0.008],
    )
    support_2_holder_H = config_vector(
        tool_visualization,
        "camera_support_2_holder_H",
        [0.010, 0.012, -0.008],
    )
    support_1_camera_H = config_vector(
        tool_visualization,
        "camera_support_1_camera_H",
        [0.035, -0.012, -0.008],
    )
    support_2_camera_H = config_vector(
        tool_visualization,
        "camera_support_2_camera_H",
        [0.035, 0.012, -0.008],
    )
    camera_support_half_thickness = float(
        tool_visualization.get("camera_support_half_thickness", 0.004)
    )
    camera_support_rgba = str(
    ...
    camera_lens_start_H = config_vector(
        tool_visualization,
        "camera_lens_start_H",
        [0.050, 0.0, 0.010],
    )
    camera_lens_end_H = config_vector(
        tool_visualization,
        "camera_lens_end_H",
        [0.050, 0.0, 0.043],
    )
    camera_lens_radius = float(
        tool_visualization.get("camera_lens_radius", 0.016)
    )
    ...
    ET.SubElement(holder_body, "geom", {
        ...
        "type": "box",
        "pos": vector_string(holder_center_H),
    ...
    rod_center_H = 0.5 * (stick_mount_H + rod_end_H)
    measured_rod_length = float(np.linalg.norm(rod_end_H - stick_mount_H))
    expected_rod_length = 2.0 * float(rod_half_size[2])
    if not math.isclose(
        measured_rod_length,
        expected_rod_length,
        rel_tol=0.0,
        ...
        raise ValueError(
            "rod_half_size[2] must be half the stick_mount_H-to-rod_end_H "
            "distance."
        )
    ET.SubElement(holder_body, "geom", {
        "name": "pbvs_stick_shaft",
        "type": "box",
        "pos": vector_string(rod_center_H),
        "size": vector_string(rod_half_size),
        "rgba": ".30 .30 .32 1",
        "contype": "0",
        "conaffinity": "0",
        "group": "2",
    ...
    ET.SubElement(holder_body, "geom", {
        "name": "pbvs_stick_sphere",
        "type": "sphere",
        "pos": vector_string(sphere_center_H),
        "size": f"{stick_radius:.10g}",
        "rgba": ".88 .88 .75 1",
        "contype": "0",
        "conaffinity": "0",
        "group": "2",
    ...
    camera_housing_body = ET.SubElement(holder_body, "body", {
        "name": "pbvs_camera_housing_body",
        "pos": vector_string(camera_body_center_H),
    })
    ET.SubElement(camera_housing_body, "geom", {
    ...
    # The calibrated optical frame is independent of the housing visual.
    ...
    ET.SubElement(tip_body, "geom", {
        "name": "pbvs_stick_tip_marker", "type": "sphere", "size": ".003",
...
class HeadlessViewer:
    """Minimal viewer interface for finite-duration, no-OpenGL validation."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="pbvs_ee",
        help=(
            "Controlled EE body. The default creates a virtual libfranka E "
            "frame 3.6 mm behind MuJoCo Menagerie's hand body."
        ),
    )
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Run without an OpenGL viewer for automated validation.",
    )
    parser.add_argument(
        "--headless-duration",
        type=float,
    ...
    if args.headless_duration <= 0.0:
        parser.error("--headless-duration must be positive.")
    ...
    # The rigid fixture is installed with 38 mm total finger separation.
    finger_distance = float(
        tool_visualization.get("finger_distance", 0.038)
    )
    if not 0.0 <= finger_distance <= 0.08:
        raise ValueError(
            "tool_visualization.finger_distance must be in [0, 0.08] m."
        )
    finger_joint_position = 0.5 * finger_distance
    for finger_joint_name in ("finger_joint1", "finger_joint2"):
        ...
        if finger_joint_id < 0:
            raise RuntimeError(
                f"MuJoCo joint {finger_joint_name!r} was not found."
            )
        finger_qpos_address = int(model.jnt_qposadr[finger_joint_id])
        finger_minimum, finger_maximum = model.jnt_range[finger_joint_id]
        data.qpos[finger_qpos_address] = np.clip(
    ...
    if gripper_actuator_id >= 0:
        gripper_minimum, gripper_maximum = model.actuator_ctrlrange[
            gripper_actuator_id
        ]
        data.ctrl[gripper_actuator_id] = (
            gripper_minimum
            + (finger_joint_position / 0.04)
            * (gripper_maximum - gripper_minimum)
        )
    ...
    try:
        viewer_context = (
            nullcontext(HeadlessViewer(args.headless_duration))
            if args.headless
