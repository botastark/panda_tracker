...
namespace
{
  ...
  struct SafetyConfig
  {
    ...
    // Explicit model for the complete physical flange payload. This controller
    // uses it only after the operator supplies --apply-load-model.
    bool load_model_enabled{false};
    bool load_model_includes_end_effector{false};
    double load_mass_kg{0.0};
    std::array<double, 3> load_com_F_m{};
    std::array<double, 9> load_inertia_at_com_F_kg_m2{};
    double load_settle_s{1.0};
    double max_preexisting_ee_mass_kg{0.01};
  };
  ...
  std::array<double, N> parse_array(std::string text, const std::string &key)
  {
    ...
    {
      throw std::runtime_error("safe config key '" + key + "' must be an array");
    }
    ...
    while (std::getline(stream, item, ','))
    {
      ...
      {
        throw std::runtime_error("safe config key '" + key + "' must contain " +
  ...
  SafetyConfig load_safety_config(const std::string &path)
  {
    ...
    s.max_abs_joint_torque_nm = parse_array<7>(require("max_abs_joint_torque_nm"), "max_abs_joint_torque_nm");
    s.max_abs_external_joint_torque_nm = parse_array<7>(require("max_abs_external_joint_torque_nm"), "max_abs_external_joint_torque_nm");
    s.max_abs_joint_torque_rate_nmps = parse_array<7>(require("max_abs_joint_torque_rate_nmps"), "max_abs_joint_torque_rate_nmps");
    ...
    s.load_model_enabled = parse_bool(require("load_model_enabled"), "load_model_enabled");
    s.load_model_includes_end_effector = parse_bool(
        require("load_model_includes_end_effector"), "load_model_includes_end_effector");
    s.load_mass_kg = parse_double(require("load_mass_kg"), "load_mass_kg");
    s.load_com_F_m = parse_array<3>(require("load_com_F_m"), "load_com_F_m");
    s.load_inertia_at_com_F_kg_m2 = parse_array<9>(
        require("load_inertia_at_com_F_kg_m2"), "load_inertia_at_com_F_kg_m2");
    s.load_settle_s = parse_double(require("load_settle_s"), "load_settle_s");
    s.max_preexisting_ee_mass_kg = parse_double(
        require("max_preexisting_ee_mass_kg"), "max_preexisting_ee_mass_kg");
    ...
    if (s.load_model_enabled)
    {
      if (!positive(s.load_mass_kg) || !positive(s.load_settle_s) ||
          s.max_preexisting_ee_mass_kg < 0.0)
      {
        throw std::runtime_error("Enabled load model has an invalid mass or timing value");
      }
      for (double value : s.load_com_F_m)
      {
        if (!std::isfinite(value))
          throw std::runtime_error("Load COM must be finite");
      }
      for (double value : s.load_inertia_at_com_F_kg_m2)
      {
        if (!std::isfinite(value))
          throw std::runtime_error("Load inertia must be finite");
      }
      const auto &I = s.load_inertia_at_com_F_kg_m2; // Franka column-major.
      const double symmetry_tolerance = 1e-9;
      if (std::abs(I[1] - I[3]) > symmetry_tolerance ||
          std::abs(I[2] - I[6]) > symmetry_tolerance ||
          std::abs(I[5] - I[7]) > symmetry_tolerance)
      {
      ...
      if (!positive(I[0]) || !positive(det2) || !positive(det3) ||
          I[0] > I[4] + I[8] || I[4] > I[0] + I[8] || I[8] > I[0] + I[4])
      {
  ...
  void print_help(const char *argv0)
  {
    std::cout
        ...
        << "  --apply-load-model    Explicitly write configured flange load to robot\n"
  ...
  Options parse_options(int argc, char **argv)
  {
    ...
    for (int i = 1; i < argc; ++i)
    {
      ...
      if (arg == "--robot-ip")
        o.robot_ip = next("--robot-ip");
      else if (arg == "--tracker-bind-ip")
        o.tracker_bind_ip = next("--tracker-bind-ip");
      else if (arg == "--tracker-source-ip")
        o.tracker_source_ip = next("--tracker-source-ip");
      else if (arg == "--tracker-port")
        o.tracker_port = parse_port(next("--tracker-port"));
      else if (arg == "--pbvs-config")
        o.pbvs_config_path = next("--pbvs-config");
      else if (arg == "--safe-config")
        o.safe_config_path = next("--safe-config");
      else if (arg == "--enable-motion")
        o.enable_motion = true;
      else if (arg == "--recover")
        o.recover = true;
      else if (arg == "--apply-load-model")
        o.apply_load_model = true;
      else if (arg == "--preflight-only")
        o.preflight_only = true;
    ...
    if (o.pbvs_config_path.empty())
      throw std::invalid_argument("--pbvs-config is required");
    if (o.safe_config_path.empty())
  ...
  std::array<double, 16> row_major_to_franka(const Transform &transform)
  {
    ...
    for (std::size_t row = 0; row < 4; ++row)
    {
      for (std::size_t col = 0; col < 4; ++col)
        result[col * 4 + row] = transform[row * 4 + col];
    }
    return result;
  }
  ...
  struct TrackerGate
  {
    ...
    static double stddev(const std::deque<QualitySample> &samples, bool angles, std::size_t axis)
    {
      ...
      for (const auto &sample : samples)
        mean += angles ? sample.rpy[axis] : sample.p[axis];
    ...
    bool process_new(const TrackerSnapshot &tracker, const SafetyConfig &s)
    {
      ...
      while (window.size() > s.quality_window_samples)
        window.pop_front();
  ...
  const char *stop_reason_text(StopReason reason)
  {
    ...
    case StopReason::kArmTimeout:
      return "arming timeout";
    case StopReason::kRuntime:
      return "motion runtime reached";
    case StopReason::kTrackingLost:
      return "tracking lost after arming";
    case StopReason::kCommandStale:
      return "PBVS command stale";
    case StopReason::kCommandLead:
      return "X command lead exceeded";
    case StopReason::kTravelEnvelope:
      return "measured flange travel envelope exceeded";
    case StopReason::kBrakingBoundary:
      return "X braking boundary reached";
    case StopReason::kJointSpeed:
      return "joint speed limit exceeded";
    case StopReason::kJointTorque:
      return "measured joint torque limit exceeded";
    case StopReason::kExternalJointTorque:
      return "external joint torque limit exceeded";
    case StopReason::kJointTorqueRate:
      return "joint torque-rate limit exceeded";
    case StopReason::kExternalWrench:
      return "external Cartesian wrench limit exceeded";
    case StopReason::kRobotContact:
      return "robot contact/collision signal";
    case StopReason::kCommunication:
      return "control command success rate too low";
  ...
  franka::RobotState apply_and_verify_load_model(franka::Robot &robot,
                                                 const SafetyConfig &s)
  {
    const franka::RobotState before = robot.readOnce();
    if (s.load_model_includes_end_effector &&
        before.m_ee > s.max_preexisting_ee_mass_kg)
    {
      throw std::runtime_error(
          "Refusing to apply combined Hand+tool load: robot already reports m_ee=" +
          std::to_string(before.m_ee) + " kg; this would double-count the end effector");
    }

    std::cout << std::setprecision(9)
              << "Applying ESTIMATED flange load: mass=" << s.load_mass_kg
              << " kg COM_F=[" << s.load_com_F_m[0] << ", "
              << s.load_com_F_m[1] << ", " << s.load_com_F_m[2] << "] m\n";
    robot.setLoad(s.load_mass_kg,
                  s.load_com_F_m,
                  s.load_inertia_at_com_F_kg_m2);
    std::this_thread::sleep_for(std::chrono::duration<double>(s.load_settle_s));

    const franka::RobotState after = robot.readOnce();
    constexpr double kMassToleranceKg = 1e-5;
    constexpr double kComToleranceM = 1e-5;
    if (std::abs(after.m_load - s.load_mass_kg) > kMassToleranceKg)
    {
      throw std::runtime_error("Robot did not report the requested load mass after setLoad");
    }
    for (std::size_t i = 0; i < 3; ++i)
    {
      if (std::abs(after.F_x_Cload[i] - s.load_com_F_m[i]) > kComToleranceM)
      {
        throw std::runtime_error("Robot did not report the requested load COM after setLoad");
      }
    }
    std::cout << "Configured load model accepted by robot; checking measured residuals.\n";
    return after;
  }
...
int main(int argc, char **argv)
{
  try
  {
    ...
    if (options.apply_load_model && !safety.load_model_enabled)
    {
      throw std::runtime_error("--apply-load-model was supplied but the safety config disables it");
    }
    if (safety.load_model_enabled && !options.apply_load_model)
    {
      throw std::runtime_error(
          "This safety config requires the estimated load model; add --apply-load-model");
    }
    ...
    if (options.preflight_only)
    {
      ...
      const franka::RobotState state = options.apply_load_model
                                           ? apply_and_verify_load_model(robot, safety)
                                           : robot.readOnce();
    ...
    try
    {
      ...
      const franka::RobotState initial_state = options.apply_load_model
                                                   ? apply_and_verify_load_model(robot, safety)
                                                   : robot.readOnce();
      {
        std::lock_guard<std::mutex> log_lock(log_mutex);
      ...
      robot.control(
          [&](const franka::RobotState &state, franka::Duration period) -> franka::CartesianVelocities
          {
            ...
            if (ever_armed && !stopping)
              motion_elapsed_s += dt;
            if (stopping)
              stopping_elapsed_s += dt;
            ...
            if (!ever_armed && control_elapsed_s >= safety.max_arm_wait_s)
              begin_stop(StopReason::kArmTimeout);
            if (ever_armed && motion_elapsed_s >= safety.max_motion_runtime_s)
            ...
            if (control_elapsed_s >= safety.torque_monitor_grace_s)
            {
              for (std::size_t i = 0; i < 7; ++i)
              {
                ...
                if (std::abs(state.tau_J[i]) > safety.max_abs_joint_torque_nm[i])
                {
                  ...
                  stop_diagnostics.joint_index = i;
                  stop_diagnostics.joint_value = state.tau_J[i];
                  break;
                }
                if (std::abs(state.tau_ext_hat_filtered[i]) > safety.max_abs_external_joint_torque_nm[i])
                {
                  ...
                  stop_diagnostics.joint_index = i;
                  stop_diagnostics.joint_value = state.tau_ext_hat_filtered[i];
                  break;
                }
                if (std::abs(state.dtau_J[i]) > safety.max_abs_joint_torque_rate_nmps[i])
                {
                  ...
                  stop_diagnostics.joint_index = i;
                  stop_diagnostics.joint_value = state.dtau_J[i];
                  break;
                }
              }
            ...
            if (torque_bad_cycles >= safety.torque_violation_cycles)
              begin_stop(torque_reason);
